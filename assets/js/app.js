...
function footer(){
return `<footer class="footer"><div class="container"><div class="footer-grid"><div><a class="logo logo-image footer-logo" href="index.html" aria-label="Kumovi Debili — početna"><img src="assets/images/logo-kumovi-debili.jpg" alt="Kumovi Debili"></a><p>Sve za momačko veče koje se pamti. Vi smislite glupost, mi je odštampamo.</p><a class="footer-instagram" href="https://www.instagram.com/kumovidebili/" target="_blank" rel="noopener noreferrer">Prati @kumovidebili na Instagramu →</a></div><div><h3>Brzi linkovi</h3><a href="shop.html">Proizvodi</a><a href="packages.html">Paketi za ekipu</a><a href="cart.html">Korpa</a></div><div><h3>Važno</h3><a href="privacy.html">Privatnost</a><a href="terms.html">Uslovi poručivanja</a><a href="contact.html">Kontakt</a><a href="mailto:kumovidebili@gmail.com">kumovidebili@gmail.com</a></div></div><div class="legal-note">© 2026 Kumovi Debili. Početna verzija sajta. Bez online plaćanja — svaka porudžbina se potvrđuje direktno.</div></div></footer>`}
function productCard(p){return `<article class="card"><a class="product-media" href="product.html?id=${encodeURIComponent(p.id)}">${p.image?`<img src="${p.image}" alt="${esc(p.name)}" loading="lazy">`:`<div class="scribble">Fotografija<br>uskoro</div>`}<span class="tag">Personalizuj</span></a><div class="card-body"><div class="meta">${esc(p.category)}</div><h3>${esc(p.name)}</h3><p>${esc(p.desc)}</p><div class="price">${esc(p.price)}</div><a class="btn alt" href="product.html?id=${encodeURIComponent(p.id)}">Personalizuj →</a></div></article>`}
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  const search=$("#shop-search");if(search)search.addEventListener("input",()=>{const q=search.value.toLowerCase();pg.innerHTML=KD_PRODUCTS.filter(p=>(p.name+" "+p.category+" "+p.desc).toLowerCase().includes(q)).map(productCard).join("")});
});
